...
class GraphProject:
    def __init__(self, num_nodes=30, directed=False , connection_prob=0.06):
...
prob_value = st.sidebar.slider("Connection Probability", 0.01, 0.20, 0.06, 0.01)
...
if st.sidebar.button("Generate New Graph 🎲"):
    st.session_state['graph'] = GraphProject(directed=is_directed, connection_prob=prob_value)
...
if graph.directed != is_directed or graph.connection_prob != prob_value:
    st.warning("Settings changed. Click 'Generate New Graph'.")
